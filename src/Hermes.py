...
from Shen import cut_ext, is_contain_chinese
...
for file_info in IELTS.find_all():
    exec('{0} = {1}'.format(file_info['filename'],
                            'File(IELTS, file_info["filename"], file_info["uploader"],file_info["type"], '
                            'file_info["downloads"], file_info["likes"])'))
...
def login():
    if request.method == 'POST':
        ...
        u = {}  # {'carlshenjc': {'username': 'carlshenjc', 'password': 'sjc123', 'full_name': 'Carl Shen'},
                # 'Jiang Zhou': {...}}
        if len(f):
            for user_info in f:
                u[user_info['username']] = user_info
            if session['userid'] in u.keys():  # 判断session刚接受的id在不在数据库返回值
                if session['userpwd'] == u[session['userid']]['password']:  # 密码在不在
...
def likes(filename):
    try:
        ...
        return """<h1 style="color: red;">Please do not do any operation to file with illegal filename</h1>
                <p>Illegal filename: filename that contains '-' or '.'</p>
                <br><br><br>
...
def upload_math():
    if request.method == 'POST':
        ...
        try:
            ...
            flash("File names contain illegal characters (no'-'or'.' in the file names)")
            return redirect('index')
        except IndexError:
            flash("File names contain illegal characters (no chinese characters file names)")
            return redirect('index')
...
def store():
    try:
        ...
        print(list)
        return render_template('store.html', list=list, points=points)
    except NameError:
...
def buy(goods_name):
    ...
    if account.reward_points >= price:
        ...
        flash('purchase successfully, you can check in your personal center, '
              'and we will deliver them to you in person weekly')
    else:
